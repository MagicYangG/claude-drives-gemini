...
> **📡 2026-08 情报:Google 正在灰度原生「去角标」**。Google App 的 APK 拆包出现 "Watermark settings" 开关,已有账号在界面见到该选项(灰度命中)。若你的账号已有此选项:原生开关 > 本文一切方案(SynthID 仍在,合规不变)。灰度铺满前 gwr 路线继续作为兜底;确认原生选项覆盖网页端后,`--dewatermark` 会转为"检测到原生开关时跳过"。
