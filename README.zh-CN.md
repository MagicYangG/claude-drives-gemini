...
- **一条命令端到端**:自动拉起专用 Chrome → 确保 Pro+扩展思考模式 → 直发提示词 → 阶段感知等待 → 原生下载 → 可选去角标,回执一行 JSON。
- **绝不双花额度**:明确拒绝才安全重发;超时只延长等待;下载失败只重下载。
- **高清无损**:2816×1536 原图原生直下;`--dewatermark` 数学无损去可见角标(SynthID 除外,见下方风险节)。
...
- 水印:仅去除可见角标(反向 Alpha,数学无损);**SynthID 溯源水印不去除也去除不了**,生成内容仍可被溯源识别。
