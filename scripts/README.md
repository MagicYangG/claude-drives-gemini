...
## ⭐ 一条龙(推荐,最省 token)
`gemini-gen.mjs` 把 open→select→wait→**原生下载**(→gwr)→关 tab 一把跑完,**只回一行 JSON**(过程噪音不进上下文):
```bash
cd ~/.claude/skills/claude-drives-gemini/scripts
node gemini-gen.mjs image "<图片提示词>" out/图.png --dewatermark   # → {"ok":true,"dims":"2816x1536","mb":10.6,...}(实际尺寸随宽高比略浮动)
...
- **本地代理**(config `proxy`)—— `gemini-media-dl` 兜底(cookie+curl 直链,node fetch 不走代理故用 curl --proxy)与 `setup-gwr` 的 git clone 使用;Chrome 自身访问 Google 走启动器内的 `--proxy-server`;原生下载主线不涉及,也**不做前置代理自检**。
- **gwr 去水印 CLI**(外部,可自动 clone):config `gwrDir`(默认 `~/gemini-watermark-remover`,`setup-gwr.mjs` 自动就位),见 `../references/dewatermark.md`。
- **勿装去水印类扩展/油猴**(Gemini Watermark Remover 等):会把下载劫持成 `blob:null`,高清自动化失效(需人工点或卸载)。原生「下载完整尺寸的图片」直出带水印 **2816** 原图,自动化用 `--dewatermark`(gwr 去角标)。

## 脚本清单
| 脚本 | 作用 |
|---|---|
| ⭐ `gemini-gen.mjs <type> "<prompt>" <out> [--dewatermark] [--use-tool] [--maxSec N] [--downloadDir DIR] [--ws WS] [--keep] [--no-preflight]` | **一条龙(推荐)**:preflight→open→直发提交→wait(阶段感知重试)→原生下载→gwr→关tab,只回一行 JSON(失败含 stage/tid) |
| `gemini-download.mjs <wsUrl> <tid> <type> <out> [--dewatermark] [--downloadDir DIR] [--profileDir DIR]` | **统一原生下载器**:点原生下载钮→轮询目录落盘(图直接/视频直接/乐选MP3),取代 cookie+curl |
...
node gemini-download.mjs "$WS" "$TID" image "out/图片.png" --dewatermark --downloadDir "$DL"   # hires 仅图片兜底
...
- 视频/音乐**不去水印**(保留 veo 角标);图片用 gwr CLI 去角标(原生下载直出带水印原图;**勿安装去水印类扩展/油猴**,装过请卸载;canvas 管道仅备用)。

## 补充脚本(2026-06-14)
| 脚本 | 作用 |
|---|---|
| `setup-gwr.mjs [dir] [proxy]` | 确保 gwr 去水印 CLI 就位;缺则走代理 clone+装依赖(幂等,防误删自愈) |
| `gemini-image-hires.mjs <wsUrl> <tid> <out> [--dewatermark] [--downloadDir DIR] [--profileDir DIR]` | **图片兜底**(功能已被 `gemini-download image` 涵盖):点原生下载→轮询下载目录→真 **2816×1536**;下载目录按 ws 端口自动判断,自定义了目录才需 `--downloadDir` |
| `gemini-image-grab.mjs <wsUrl> <tid> <out> [--dewatermark]` | 备用(预览):canvas→toDataURL 取无损 PNG,但只 **1408×768** 半分辨率(字节不进上下文) |
...
简化后图片全自动一条龙(**高清 2816**):
```bash
node setup-gwr.mjs                                  # 确保去水印工具在
WS=$(node gemini-wsurl.mjs); TID=$(node gemini-open.mjs "$WS")
node gemini-select-type.mjs "$WS" "$TID" image "<图片提示词>"
node gemini-wait.mjs "$TID" image 180 "$WS"
node gemini-download.mjs "$WS" "$TID" image out/图片.png --dewatermark --downloadDir "$HOME/Downloads"
