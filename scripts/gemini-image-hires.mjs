...
// 用法(旧签名): node gemini-image-hires.mjs <wsUrl> <tid> <out> [--downloadDir DIR] [--profileDir DIR]
import { execFileSync } from 'node:child_process';
import { dirname, join } from 'node:path';
import { fileURLToPath } from 'node:url';
const here = dirname(fileURLToPath(import.meta.url));
const [ws, tid, out, ...rest] = process.argv.slice(2);
if (!ws || !tid || !out) { console.log('用法: node gemini-image-hires.mjs <wsUrl> <tid> <out> [--downloadDir DIR] [--profileDir DIR]'); process.exit(1); }
