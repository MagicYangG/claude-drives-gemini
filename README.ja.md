...
- **コマンド 1 本で完結**:専用 Chrome の自動起動 → Pro + 拡張思考モードの確認 → プロンプト送信 → 段階を認識した待機 → ネイティブダウンロード → 任意でウォーターマーク除去。戻り値は 1 行の JSON。
- **クォータを二重消費しない**:明確な拒否のときだけ安全に再送信し、タイムアウトは待機を延長するだけ、ダウンロード失敗はダウンロードのみ再試行。
- **フル解像度・ロスレス**:2816×1536 のオリジナルをネイティブに直接ダウンロード。`--dewatermark` は可視ロゴを数学的に無損失で除去(SynthID は対象外 — リスクの節を参照)。
...
- ウォーターマーク:除去されるのは可視ロゴのみ(逆アルファ合成、数学的に無損失)。**SynthID の来歴ウォーターマークは除去されず、また除去できません**。生成物は引き続き追跡可能です。
